...
class _ScorerState extends State<Scorer> {
  ...
  void initState() {
    super.initState();
    final MatchEventViewModel initialState = Provider.of<MatchEventViewModel>(context, listen: false);
  ...
  List<Widget> getHomeGoals({MatchEventViewModel model}) {
    final List<Widget> homeTeamGoals = model.events
        .where(
            (event) => (event.team == widget.score.homeTeam && event.type == "Goal"))
        .map((event) {
      return Padding(
  ...
  List<Widget> getAwayGoals({MatchEventViewModel model}) {
    final List<Widget> awayTeamGoals = model.events
        .where(
            (event) => (event.team == widget.score.awayTeam && event.type == "Goal"))
        .map((event) {
      return Padding(
        padding: const EdgeInsets.all(2.0),
        child: Text(
